...
The cron used to `kinit` with a dedicated per-user cron keytab (`kcroninit`, then `kinit -kt /path/to/cron.keytab <user>/cron/<host>@FNAL.GOV` before every run) so it could authenticate without an interactive login, since a normal `sbndpro` login was assumed off-limits for cron. In practice this backfired: on `sbndgpvm01`, a bad Kerberos ticket produced by that cron flow interfered with the same user's own interactive home-area access.
