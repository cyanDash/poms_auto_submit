...
def test_next_slice_count_bootstraps_single_target_when_nothing_submitted():
    num = psc._next_slice_count(make_cfg(submit_two_slices=False), make_submissions())
    assert num == 1


def test_next_slice_count_bootstraps_double_target_when_nothing_submitted():
    num = psc._next_slice_count(make_cfg(submit_two_slices=True), make_submissions())
    assert num == 2
...
def test_next_slice_count_skips_when_max_splits_reached():
    num = psc._next_slice_count(make_cfg(last_split=5, max_splits=5), make_submissions())
    assert num == 0


def test_next_slice_count_skips_when_last_split_past_max_splits():
    num = psc._next_slice_count(make_cfg(last_split=6, max_splits=5), make_submissions())
    assert num == 0


def test_next_slice_count_caps_to_remaining_splits_when_bootstrapping():
    num = psc._next_slice_count(make_cfg(submit_two_slices=True, last_split=4, max_splits=5), make_submissions())
    assert num == 1
